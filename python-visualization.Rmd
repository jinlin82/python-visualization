...
1.  Matplotlib 具有两种不同的接口. 第一种是面向对象的接口 (OO interface)
    . 使用这种接口,我们创建并使用Axes类对象的实例.
	从而利用Axes对象返回一个可视化效果于一个Figure类对象的实例上.
2.  第二种基于Matlab,并使用一套基于状态的接口. 所有这些功能都被封装于Pyplot模块中供用户使用.
3.  大多数术语与对象方法的名称都十分直接好懂,但仍然有一些需要注意:
    1.  Figure是最终呈现的图像的对象,包含一个或多个Axes.
    2.  每个Axes对象都代表一个独立的图 (别与 \"axis\"这个词混淆了, 这个指的是一个图的x/y座标轴).
    3.  我们直接调用Axes对象的方法来进行绘图,
        这使得我们的自定义绘图的过程更加灵活与强大.
...
1.  为了使用面向对象塞方式与之交互, 我们首先需要
    生成一个Figure对象的实例.Figure 与 axes.Axes.
2.  Figure 对象如同一个画布, Axes 即是这个画布的一部分
    一个特定的可视化效果的一部分.
3.  Figures 可以由多个Axes堆叠组成.
...
2.  该函数接受一个或多个Matplotlib objects属性的列表, 并尝试为每个对象设置相应的样式元素.
...
1.  让atplotlib自动为我们在Figure对象上创建的元素留出空间.
...
1.  给图像添加标签(label).若要用用面向对象接口, 
	我们可以使用axes.Axes.set()这个方法 去设置这些 Axes 对象的属性.
2.  用pyplot.subplots()函数更改图的大小.
    我们可以通过修改figsize这个关键词参数(**kwrag)来实现.
3.  对于标签(label), 我们可以通过使用函数选择自定义的标识栏,使用ticker.FuncFormatter类来完成你的修改.
...
2.  要做到这些,我们仅需要调用这个axes对象中的绘图方法即可.
...
1.  在Matplotlib中,我们可以把绘图结果保存成多种文件格式. 若你想了解那些格式可用,可以使用以下函数查看:
...
2.  这里有几个有用的参数:
    ...
    3.  bbox~inches~=\"tight\" 选择保存图像的边界拟合方式
...
2. 	Matplotlib提供了很多预先定义好的样式.举个例子,其中一个样式名为\"ggplot\"
	能模拟ggplot的包的美学风格(ggplot是一个用于R语言的十分流行的绘图包).
...
1.  要查看所有可用样式: `print(plt.style.available)`

### 定义自定义样式

1.  你可以创建自定义样式,并通过调用style.use函数并加上样式文件的URL或文件路径来调用之.
2.  并且,如果你将你的\<style-name\>.mplstyle 样式文件添加到
    mpl~configdir~/stylelib文件夹下, 你可以直接使用style.use(\<style-name\>)去重复调用你的样式文件.

### 样式组合

1.  样式文件本就是被设计为组合在一起使用的.
2.  所以你可以用一个样式文件设定颜色,并用另一个样式文件去设定展示文件的元素大小.
3.  这些样式很容易被组合,你仅需要传入一个由样式组成的列表即可:
4.  注意,左侧定义的样式(如果与右侧的样式有冲突)会被右侧的样式覆盖(冲突的部分).
...
1.  如果你想为指定的一块代码使用特定的样式,但又不希望改动全局样式,你可以使用style
	包提供的内容管理器去限制你的修改,让他作用与于一个特定的词法作用域内.使用如
	下的代码去将你的样式修改独立出来
...
2.  All of the rc settings are stored in a dictionary-like variable
    ...
    package
	所有的 rc 文件设置(参数)都被存储与一个类似_字典_的数据中,名叫matplotlib.rcParams,这个变量
	对于matplotlib是一个全局变量.
3.  rcParams可以被直接修改,如下:
...
3.  设置rcParams的数值时,会有一定程度的验证措施,请参阅matplotlib.rcsetup了解更多.
...
1.  matplotlib使用matplotlibrc 配置文件来设置各种属性,我们称之为rc 设置或rc 参数.
2.  你可以控制Matplotlib几乎所有的默认数值:
	图像大小,分辨率,行高,颜色和样式,axes,座标轴,网格参数,文本与字体属性等等.
3.  当一个matplotlibrc文件被找到,其他的matplotlibrc文件的路径便不再被搜索.
4.  用以下方法查看哪个文件路径下的matplotlibrc文件被加载了:
